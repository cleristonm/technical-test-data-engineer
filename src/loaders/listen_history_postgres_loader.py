...
class ListenHistoryPostgresLoader(BasePostgresLoader):
    ...
    def load(self, table_name: str, data: List[Dict[str, Any]]) -> None:
        if not data:
            self.log.info(f"No data to load for listen history")
            return
        ...
        try:
            with psycopg2.connect(**self.connection_params) as conn:
                with conn.cursor() as cur:
                    ...
                    if invalid_records:
                        self.log.warning(f"Found {len(invalid_records)} records with non-existent user_ids:")
                        for record in invalid_records:
                            self.log.warning(f"Skipping record: user_id={record['user_id']}, track_id={record['track_id']}, updated_at={record['updated_at']}")
                    ...
                    values = [[row[column] for column in columns] for row in valid_records]
                    
                    self.log.info(f"Starting to load {len(valid_records)} valid listen history records")
                    ...
                    self.log.info(f"Successfully loaded {len(valid_records)} listen history records")
