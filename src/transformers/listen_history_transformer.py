...
class ListenHistoryTransformer(BaseTransformer):
    ...
    def _transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            raise TransformerError(f"Listen history transformation failed: {str(e)}") from e
    ...
    def _expand_track_listings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Args:
            df: DataFrame with columns [user_id, items (list), created_at, updated_at]
            
        Returns:
            pd.DataFrame: Expanded DataFrame with columns [user_id, track_id, created_at, updated_at]
        ...
        valid_mask = (
            df['user_id'].notna() & 
            df['items'].apply(lambda x: isinstance(x, list) and len(x) > 0)
        )
        
        if not valid_mask.any():
            return pd.DataFrame(columns=['user_id', 'track_id', 'created_at', 'updated_at'])
        
        # Explode the items list into separate rows
        expanded_df = df[valid_mask].explode('items').rename(columns={'items': 'track_id'})
        
        # Validate track_ids
        expanded_df['track_id'] = pd.to_numeric(expanded_df['track_id'], errors='coerce')
        ...
        if not valid_tracks.all():
            self._validation_errors.extend(
                ...
                for user_id, track_id in expanded_df[~valid_tracks][['user_id', 'track_id']].values
            )
        
        return expanded_df[valid_tracks][['user_id', 'track_id', 'created_at', 'updated_at']]